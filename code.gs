/**
 * みらいパスポート v1.0
 * "Mirai Compass" Design System Integration
 * Copyright 2026 GIGA App Architect
 */
...
/**
 * セットアップ状況の確認
 * ScriptProperties(アプリ全体の設定)を確認する
 */
...
function performInitialSetup() {
  ...
  try {
    ...
    ensureSheet(ss, 'Responses', ['responseId', 'taskId', 'studentId', 'studentName', 'submittedAt', 'canvasImage', 'textContent', 'status', 'feedbackText', 'score', 'feedbackJson']);
...
function saveStudentResponse(data) {
  ...
  if(row > 0) {
    ...
    if(data.canvasJson) sheet.getRange(row, 11).setValue(data.canvasJson);
  } else {
    sheet.appendRow([Utilities.getUuid(), data.taskId, data.studentId, data.studentName, new Date(), data.canvasImage||"", data.textContent||"", data.status||"submitted", "","", data.canvasJson||""]);
  }
  return { success: true };
}
...
function saveFeedback(data) {
  ...
  if(data.rowIndex) {
    ...
    if(data.canvasJson) sheet.getRange(data.rowIndex, 11).setValue(data.canvasJson);
  }
  return { success: true };
}
...
function getSharedResponses(taskId) {
  const sheet = getDbSpreadsheet().getSheetByName('Responses');
  const values = sheet.getDataRange().getValues();
  return values.filter(r => String(r[1]) === String(taskId) && (r[7] === 'submitted' || r[7] === 'graded')).map(r => ({ studentName: r[3], canvasImage: r[5] }));
}
...
function getMyResponse(taskId, studentId) {
  ...
  for(let i=values.length-1; i>=1; i--){
    if(String(values[i][1]) === String(taskId) && String(values[i][2]) === String(studentId)) {
      return { responseId: values[i][0], status: values[i][7], feedbackText: values[i][8], feedbackJson: values[i][10], canvasImage: values[i][5], canvasJson: values[i][10] };
    }
  }
  return null;
}
...
function createPdf(html, title) {
  try {
    const t = title || "Worksheet";
    const s = `<html><head><style>body{font-family:sans-serif;padding:40px;} .ws-header{text-align:right;border-bottom:1px solid #000;margin-bottom:20px;} h1{text-align:center;border-bottom:3px double #000;padding-bottom:10px;} .ws-box{border:1px solid #000;padding:20px;min-height:100px;margin:15px 0;} table{width:100%;border-collapse:collapse;} td,th{border:1px solid #000;padding:10px;} img{max-width:100%;}</style></head><body>${html||""}</body></html>`;
    const b = Utilities.newBlob(s, 'text/html', t+'.html');
    const f = DriveApp.createFile(b);
    const p = f.getAs('application/pdf').setName(t+".pdf");
    f.setTrashed(true);
    const pf = DriveApp.createFile(p);
    return { url: pf.getUrl(), downloadUrl: pf.getDownloadUrl() };
  } catch (e) { throw new Error("PDF作成エラー: " + e.message); }
}
